...
class Settings:
    ...
    AI_PIPELINE_VERSION: str = os.getenv("AI_PIPELINE_VERSION", "2.6")
    ...
    # Part 2.5/2.6: Intelligence & Telemetry (all offline by default)
    AI_FALLBACK_TO_MOCK: bool = os.getenv("AI_FALLBACK_TO_MOCK", "true").lower() not in ("false", "0", "no")
    AI_EMBEDDING_PROVIDER: str = os.getenv("AI_EMBEDDING_PROVIDER", "mock")
    AI_EMBEDDING_MODEL: str = os.getenv("AI_EMBEDDING_MODEL", "mock-16")
    AI_EMBEDDING_DIMENSION: int = int(os.getenv("AI_EMBEDDING_DIMENSION", "16"))
    AI_VECTOR_STORE: str = os.getenv("AI_VECTOR_STORE", "memory")
    VECTOR_STORE: str = os.getenv("VECTOR_STORE", "memory")  # alias
    PGVECTOR_COLLECTION: str = os.getenv("PGVECTOR_COLLECTION", "historical_cases")
    AI_OCR_PROVIDER: str = os.getenv("AI_OCR_PROVIDER", "mock")
    AI_POLICY_RETRIEVAL_TOP_K: int = int(os.getenv("AI_POLICY_RETRIEVAL_TOP_K", "5"))
    AI_HISTORICAL_RETRIEVAL_TOP_K: int = int(os.getenv("AI_HISTORICAL_RETRIEVAL_TOP_K", "5"))
    AI_EVALUATION_ENABLED: bool = os.getenv("AI_EVALUATION_ENABLED", "true").lower() not in ("false", "0", "no")
    AI_TELEMETRY_ENABLED: bool = os.getenv("AI_TELEMETRY_ENABLED", "true").lower() not in ("false", "0", "no")
