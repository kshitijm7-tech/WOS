...
const nav = [
  ...
  { to: "/customer/products", label: "My Products" },
  { to: "/customer/claims", label: "My Claims" },
  { to: "/customer/claims/new", label: "New Claim", highlight: true },
];
    ...
    <div className="min-h-screen bg-paper flex">
      ...
      <aside className="hidden lg:flex w-64 bg-ink text-paper flex-col sticky top-0 h-screen">
        <div className="px-6 py-6 border-b border-slate-dark/30">
          ...
          <p className="text-xs text-slate-light mt-1 font-mono">Customer Portal</p>
        </div>
        ...
        <div className="p-4 border-t border-slate-dark/30">
          ...
          <div className="text-xs text-slate-light truncate">{user?.email}</div>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-30 bg-ink text-paper flex items-center justify-between px-4 py-3">
        <Link to="/customer/dashboard" className="font-display font-semibold">
          Warranty<span className="text-teal">OS</span>
        </Link>
          ...
          <nav className="px-4 py-4 space-y-1">
            {nav.map((item) => (
              <NavLink
                ...
                className="block px-3 py-3 rounded-md text-paper hover:bg-white/10"
              >
                {item.label}
              </NavLink>
            ))}
